...
3. 「新規テクスチャに保存」で `xxx_new.png` として書き出し、材質の参照先を変更します（ワンクリック・PNG 元サイズ）。
   形式やサイズを選ぶ場合は「新規テクスチャに保存…（形式とサイズを選択）」を使います。
...
- **プリセット管理**：名前変更 / 削除 / 並び替え（名前順 → 更新順 → 手動）。「上へ / 下へ」は自動的に手動順序へ切替（`presets\order.txt` に保存）。
...
- **マスク・UV レイアウト書き出し**：選択範囲マスク / 現在の材質マスク / 全材質マスク（白黒 PNG）、
  **アルファ通道付きの選択範囲マスク**：alpha が選択範囲そのもの（選択 = 不透明）。RGB は 白一色 / 元テクスチャ / 調整結果 から選択でき、Photoshop のレイヤーマスクやチャンネルにそのまま使えます。
...
書き出し（「新規テクスチャに保存…」）も同じ形式に対応：PNG / JPG / BMP / GIF / TIFF は GDI+、TGA と DDS（無圧縮·DXT1/3/5）は同梱の自作エンコーダー。サイズは 元サイズ / 1/2 / 1/4 / カスタム から選択できます。
