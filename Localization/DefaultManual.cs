...
namespace TextureGrade.Localization
{
    ...
    public static class DefaultManual
    {
        ...
        private static string ZhCn() => new StringBuilder()
            ...
            .AppendLine("· 导出选区蒙版…（带 Alpha 通道 PNG）：alpha 就是选区（选中 = 不透明），RGB 可选")
            .AppendLine("  纯白 / 原贴图 / 调色结果 —— 拖进 Photoshop 直接当图层蒙版或通道用")
            .AppendLine("· 另存为新贴图…（选格式与尺寸）：PNG / JPG / BMP / GIF / TIFF / TGA / DDS（未压缩·DXT1/3/5），")
            .AppendLine("  可选原尺寸 / 1/2 / 1/4 / 自定义宽高；JPG 可调质量。换格式时扩展名会自动跟着改")
        ...
        private static string ZhTw() => new StringBuilder()
            ...
            .AppendLine("· 匯出選取遮罩…（含 Alpha 通道 PNG）：alpha 就是選取（選取 = 不透明），RGB 可選")
            .AppendLine("  純白 / 原貼圖 / 調色結果 —— 拖進 Photoshop 直接當圖層遮罩或通道用")
            .AppendLine("· 另存為新貼圖…（選格式與尺寸）：PNG / JPG / BMP / GIF / TIFF / TGA / DDS（未壓縮·DXT1/3/5），")
            .AppendLine("  可選原尺寸 / 1/2 / 1/4 / 自訂寬高；JPG 可調品質。換格式時副檔名會自動跟著改")
        ...
        private static string En() => new StringBuilder()
            ...
            .AppendLine("- Export Selection Mask… (Alpha channel PNG): the alpha channel *is* the selection (selected = opaque);")
            .AppendLine("  RGB can be plain white / the original texture / the graded result — drop it straight into Photoshop as a layer mask or channel.")
            .AppendLine("- Save as New Texture… (pick format & size): PNG / JPEG / BMP / GIF / TIFF / TGA / DDS (raw, DXT1/3/5),")
            .AppendLine("  with original / 1:2 / 1:4 / custom width+height and a JPEG quality slider. The extension follows the format automatically.")
        ...
        private static string Ja() => new StringBuilder()
            ...
            .AppendLine("· 選択範囲マスクを書き出し…（アルファ通道付き PNG）：alpha が選択範囲そのもの（選択 = 不透明）。")
            .AppendLine("  RGB は 白一色 / 元テクスチャ / 調整結果 から選択 —— Photoshop のレイヤーマスクやチャンネルにそのまま使えます")
            .AppendLine("· 新規テクスチャに保存…（形式とサイズを選択）：PNG / JPG / BMP / GIF / TIFF / TGA / DDS（未圧縮·DXT1/3/5）、")
            .AppendLine("  サイズは 元サイズ / 1/2 / 1/4 / カスタム、JPG は品質を指定。形式を変えると拡張子も自動で追随します")
