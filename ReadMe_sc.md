...
3. 「另存为新贴图」落盘成 `xxx_new.png` 并让材质指向它（一键，PNG 原尺寸）；
   要换封装格式或缩放尺寸时用「另存为新贴图…（选格式与尺寸）」。
...
- **预设管理**：改名 / 删除 / 排序（按名称 → 按时间 → 手动顺序），上移 / 下移会自动切到手动顺序（顺序存在 `presets\order.txt`）。
- **直方图**：RGB 叠加，实时反映预览，视图菜单可关。
- **对比原图**：一键切换原图 / 调色结果（参数保留）。
- **蒙版与 UV 布局导出**：当前选区蒙版 / 当前材质蒙版 / 全部材质蒙版（黑白 PNG）、UV 布局图（透明底 + 线框 PNG）。
- **带 Alpha 通道的选区蒙版**：alpha 就是选区（选中 = 不透明），RGB 可选纯白 / 原贴图 / 调色结果，拖进 PS 直接当图层蒙版或通道。
...
写入（「另存为新贴图…」）同样覆盖这全部格式：PNG / JPG / BMP / GIF / TIFF 走 GDI+，TGA 与 DDS（未压缩·DXT1/3/5）用项目内手写编码器，并支持原尺寸 / 1/2 / 1/4 / 自定义宽高。
